...
// D. Gaskell: added some functionality to retrieve info from already existing trees
...
extern "C" {
  ...
  void initmcrootnt_(const char *title, const char *access, int ltitle, int laccess);
...
//------------------------------------------------------------------------
void readntbranch_(int *ievent)
 //Read the variables from the tree for event ievent defined using tagntbranch
{
  //  cout << *ievent;
  nt->GetEntry(*ievent);
  return ;
